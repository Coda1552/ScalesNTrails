...
public class LeafTailedGeckoEyesLayer extends GeoRenderLayer<EntityLeafTailedGecko> {
   ...
   public void render(PoseStack poseStack, EntityLeafTailedGecko entityLivingBaseIn, BakedGeoModel bakedModel, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
        ...
        if (night) {
           RenderType cameo = RenderType.eyes(OVERLAY);
            getRenderer().reRender(this.getGeoModel().getBakedModel(MODEL), poseStack, bufferSource, entityLivingBaseIn, renderType,
                   bufferSource.getBuffer(cameo), partialTick, packedLight, OverlayTexture.NO_OVERLAY, 1, 1, 1, 1);
       }
    }
